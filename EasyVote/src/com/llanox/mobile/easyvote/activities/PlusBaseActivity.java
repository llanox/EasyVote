...
import com.llanox.mobile.easyvote.data.StoreSession.ResponseHandler;
...
ConnectionCallbacks, OnConnectionFailedListener
	...
	/**
	 * Called when the PlusClient is blocking the UI. If you have a
	...
	/**
	 * Connect the only if a connection isn't already in
	...
	/**
	 * Disconnect the PlusClient only if it is connected (otherwise, it
	...
	private void saveSession() {
        ...
        createUserSession(email, id, nickName, fullname);
		
	}

    protected void createUserSession(String email, String id, String nickName, String fullname) {
                ...
                UserModel user = new UserModel(id,nickName,null,fullname,email,UserModel.GOOGLE_PLUS_USER_ROLE);
                UserStore data = new UserStore(this);
                data.asyncInsert(user, new ResponseHandler<UserModel>() {

                    @Override
                    public void succesfull(UserModel item) {
                        Toast.makeText(PlusBaseActivity.this, "UserModel saved succesfully", Toast.LENGTH_LONG).show();
                    }
